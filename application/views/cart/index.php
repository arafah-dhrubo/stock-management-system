        ...
        <?php if (count($data) > 0) { ?>
            <table class="w-full">
                ...
                foreach ($data as $key => $value) { ?>
                    <tr>
                        ...
                        <td class="px-2 py-1 border"><?php echo $value->quantity ?> <input type="number" class="form-control text-center" value="<?php echo $value->quantity; ?>" onchange="updateCart(this, '<?php echo $key; ?>')"></td>
            ...
            <div class="flex justify-between py-2"><h1 class="text-xl">Total Payable <span
                ...
                <a href="<?php echo base_url() . 'order/checkout' ?>"
                   class="text-sm border bg-indigo-500 px-3 py-2 text-white rounded uppercase">Checkout</a>
            </div>
        <?php } else {
